...
class Echart(Base):
    def __init__(self, title, description=None, axis=True, html_title="Echarts",
                 logo_url='http://echarts.baidu.com/images/logo.png', **kwargs):
        self.title = {
            'text': title,
            'subtext': description,
        }
    ...
    def _html(self):
        with open(os.path.join(os.path.dirname(__file__), 'template.html')) as f:
            template = f.read()
            return_html = template.replace('{{ opt }}', json.dumps(self.json, indent=4)).replace('{title}',
                                                                                                 self.html_title).replace(
                '{logo_url}', self.logo)
        return return_html
    ...
    def plot(self, persist=True):
        ...
        with tempfile.NamedTemporaryFile(suffix='.html', delete=not persist) as fobj:
            ...
            persist or str(input('Press enter for continue'))
